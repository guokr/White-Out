...
from .static_dict import StaticDictionary


class BrillMooreModel():
    """Model that uses statistics based error model to find best candidates in a static dictionary.
    ...
    def __init__(self, model_path: str, dictionary_path: str, window: int=1, candidates_count: int=1):
    ...
    def load(self):
        ...
        """
        if self.model_path:
            print("loading brill moore model from `{}`".format(self.model_path))
            with open(self.model_path, 'r', newline='', encoding='utf8') as tsv_file:
                reader = csv.reader(tsv_file, delimiter='\t')
                for w, s, p in reader:
                    self.costs[(w, s)] = log(float(p))

        if self.dictionary_path:
            print("loading dictionary from `{}`".format(self.dictionary_path))
            self.dictionary = StaticDictionary(dict_path=self.dictionary_path)
